...
@pytest.mark.parametrize(
    "tfilepath,root_dir,exp_upload_path",
    (
...
def test_upload_file(tfilepath, root_dir, exp_upload_path):
    ...
    with patch("openscm_zenodo.zenodo.upload_with_progress_bar") as mock_upload:
        res = upload_file(tfilepath, tbucket, tzenodo_url, ttoken, **call_kwargs,)

    assert res is None
    mock_upload.assert_called_with(tfilepath, expected_upload_url)
